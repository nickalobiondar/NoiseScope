...
| `duplicate`    | Inserts a copy after an event (replay / double delivery).     |
| `reorder`      | Swaps two events (out-of-order delivery).                     |
| `corrupt_meta` | Alters `nonce`, `seq`, or `role` of one event.                |

Mutation plans are generated by a SplitMix64 PRNG seeded from `--seed`, making
every run reproducible. Out-of-range indices (which can arise after a `drop`
shrinks the list) are treated as no-ops, keeping mutation application total.

---

## 6. Minimization

A failing plan is reduced to a **1-minimal** subsequence via a greedy
delta-debugging pass: repeatedly attempt to remove each mutation and keep the
removal whenever the transcript still diverges, iterating to a fixed point.
The report records both the original and minimized plan lengths and the number
